...
#define ESP_LCD_TOUCH_GT911_CHECKSUM_REG    (0x80FF)
#define ESP_LCD_TOUCH_GT911_FRESH_REG       (0x8100)

/* Config block is 184 bytes: 0x8047..0x80FE */
#define ESP_LCD_TOUCH_GT911_CONFIG_LEN      (184)
/* Offset of ShakeCount within the config block (0x804F - 0x8047 = 8).
 * High nibble = release debounce count, low nibble = press debounce count.
 * Each count is one chip sample period (~5 ms at default rate).
 * 0x33 → 3 samples press + 3 samples release = ~15 ms each. */
#define ESP_LCD_TOUCH_GT911_SHAKE_COUNT_OFF (8)
#define ESP_LCD_TOUCH_GT911_SHAKE_COUNT     (0x33)
...
{
    uint8_t buf[ESP_LCD_TOUCH_GT911_CONFIG_LEN];

    /* Read the full config block so we only change what we need */
    ESP_RETURN_ON_ERROR(
        touch_gt911_i2c_read(tp, ESP_LCD_TOUCH_GT911_CONFIG_REG, buf, sizeof(buf)),
        TAG, "GT911 config read failed");

    /* Set hardware debounce (ShakeCount) */
    buf[ESP_LCD_TOUCH_GT911_SHAKE_COUNT_OFF] = ESP_LCD_TOUCH_GT911_SHAKE_COUNT;

    /* Bump config version so the chip latches the new values */
    buf[0] = (buf[0] < 0xFE) ? buf[0] + 1 : 0x01;

    /* Checksum: two's complement of the byte sum */
    uint8_t checksum = 0;
    for (int i = 0; i < ESP_LCD_TOUCH_GT911_CONFIG_LEN; i++) {
        checksum += buf[i];
    }
    checksum = (~checksum) + 1;

    /* Write config block, then checksum, then trigger a config refresh */
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(tp->io, ESP_LCD_TOUCH_GT911_CONFIG_REG, buf, sizeof(buf)),
        TAG, "GT911 config write failed");
    ESP_RETURN_ON_ERROR(touch_gt911_i2c_write(tp, ESP_LCD_TOUCH_GT911_CHECKSUM_REG, checksum),
        TAG, "GT911 checksum write failed");
    ESP_RETURN_ON_ERROR(touch_gt911_i2c_write(tp, ESP_LCD_TOUCH_GT911_FRESH_REG, 0x01),
        TAG, "GT911 config fresh failed");

    ESP_LOGI(TAG, "GT911 ShakeCount=0x%02x (~%d ms press/release debounce)",
             ESP_LCD_TOUCH_GT911_SHAKE_COUNT,
             (ESP_LCD_TOUCH_GT911_SHAKE_COUNT & 0x0F) * 5);
    return ESP_OK;
}
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(touch_gt911_write_cfg(tp), TAG, "GT911 config write failed");
